...
import { ArrowLeft, QrCode, Share2, Home, Truck, ShieldCheck, Package, Users, FileText, Video, MapPin, ChevronRight } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-off-white pb-24">
            ...
            <div className="relative h-72">
                <Image src={qurbanData.image} alt={qurbanData.name} fill className="object-cover" />
            ...
            <section className="px-5 -mt-4 mb-6">
                <JedagCard>
                    <CardBody className="flex items-center justify-between mt-4">
                        <div>
            ...
            <BottomSheet open={showPassportSheet} onOpenChange={setShowPassportSheet} title="Paspor Hewan">
                ...
                <div className="relative">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                        <div className="h-16 w-16 rounded-xl bg-white shadow-md overflow-hidden">
                            <Image src={data.image} alt={data.name} width={64} height={64} className="object-cover w-full h-full" />
                        </div>
                        <div>
                            <p className="text-xs text-ink-500 font-mono">{data.id}</p>
                            <h3 className="text-xl font-bold text-ink-900">{`"${data.name}"`}</h3>
                            <div className="flex gap-1.5 mt-1">
                                <JedagBadge variant="syariah" size="sm">
                                    Syariah
                                </JedagBadge>
                                <JedagBadge variant="sehat" size="sm">
                                    Sehat
                                </JedagBadge>
                            </div>
                        </div>
                    ...
                    <div className="space-y-3">
                        {passportFields.slice(2).map((field) => (
                            <div key={field.label} className="flex justify-between items-center">
                                ...
                                <span className={`text-sm font-medium text-ink-900 ${field.mono ? 'font-mono' : ''}`}>{field.value}</span>
                            </div>
                        ))}
                    </div>
                </div>
