...
import { BottomNav, JedagCard, CardBody, JedagAvatar, JedagBadge, JedagButton } from '@/components/jedag';
import { Bell, Settings, ChevronRight, Package, Heart, FileText, HelpCircle, LogOut, Gift } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-off-white pb-24">
            ...
            <div className="px-5 -mt-16">
                <JedagCard>
                    <CardBody className="text-center">
                        <JedagAvatar fallback="Ahmad Fauzi" size="xl" className="mx-auto mt-5 border-4 border-white" />
            ...
            <div className="px-5 mt-6">
                                ...
                                {item.badge && (
                                    <span
                                        className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                                            item.badge === 'Baru' ? 'bg-emerald-100 text-emerald-700' : 'bg-ink-100 text-ink-700'
                                        }`}
                                    >
                                        {item.badge}
                                    </span>
                ...
                {/* Logout */}
                <JedagButton variant="ghost" fullWidth className="mt-4 text-error hover:bg-red-50">
                    <LogOut className="h-5 w-5" strokeWidth={1.75} />
                    Keluar
                </JedagButton>

                {/* Version */}
                <p className="text-center text-xs text-ink-400 mt-6">Jejak Daging v1.0.0</p>
            </div>
